...
export default function AdvancedFilterDemo() {
    ...
    const imageTpeBodyTemplate = (rowData)=>{
        return <Image src={`${rowData.image_tpe_routine}`} zoomSrc={`${rowData.image_tpe_routine}`}  alt={rowData.image_tpe_routine} width="80" height="60"  className="w-6rem shadow-2 border-round" preview/>;
    }
    ...
    return (
        <div className="card">
            <DataTable value={routines} paginator showGridlines rows={10} loading={loading} dataKey="id" 
                ...
                <Column field="image_tpe_routine" header="Image TPE" body={imageTpeBodyTemplate}  style={{ minWidth: '10rem' }} />
            </DataTable>
        </div>
    );
}
